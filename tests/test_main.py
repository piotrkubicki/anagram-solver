...
@pytest.mark.parametrize(
    "valid_chars,min_word_len,word,expected",
    [
...
def test_get_valid_words(mock_read_words):
    ...
    assert sorted(get_valid_words(valid_chars, 2, "test_path")) == sorted(
        ["bit", "bike"]
    )


@pytest.mark.parametrize(
    "tested,searched,expected",
    [
        ("trust", "strut", True),
        ("bike", "motorbike", True),
        ("carr", "care", False),
...
@pytest.mark.parametrize(
    "words,word_count,expected",
    [
        (
            ["s", "m", "l", "xl", "xxl"],
            [1, 2, 3],
            [("s", "xl", "xxl"), ("m", "xl", "xxl"), ("l", "xl", "xxl")],
        ),
        (
            ["s", "m", "l", "xl", "xxl"],
            [1, 2, 1],
...
@pytest.mark.parametrize(
    "num_words,min_chars,max_chars,limit,expected",
    [
        (2, 2, 6, 6, [(3, 3)]),
...
def test_find_words_lengths(num_words, min_chars, max_chars, limit, expected):
    assert set((find_words_lengths(num_words, min_chars, max_chars, limit))) == set(
        expected
    )
...
def test_find_secret():
    ...
    results = {}
    find_secret(dictionary, (3, 5, 3), 3, anagram, results, 1)
